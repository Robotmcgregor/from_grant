...
def rfrmodel():
    ...
    """
    df = pd.read_csv(r"C:\Users\rmcgr\ntg_repository\tern\outputs\indices.csv", header=0)
    df2 = df.dropna()
    # read in the selected predictor variables
        
    xdata1=df2[['psB1a', 'psB2a', 'psB3a', 'psB4a', 'psB5a', 'psB6a', 'ratio32a', 'ratio42a',
    ...
    rfrModel_1 = rfr(n_estimators=512, oob_score=True,  max_features = 'log2', min_samples_split=1.0,n_jobs=-1) 
...
def mainRoutine():
    ...
    df.to_csv(r'C:\Users\rmcgr\ntg_repository\tern\outputs/ImportanceScore_model_l8_99perc_20190407_512_n18_int32v.csv')
    
    stats = np.vstack([fmr4,fmMSE4]).T
    
    df2 = pd.DataFrame(stats, columns=['r2','mse'])
    
    df2.to_csv(r'C:\Users\rmcgr\ntg_repository\tern\outputs/Model_stats_model_l8_99perc_20190407_512_n18_int32v.csv')
    
    d = klepto.archives.file_archive(r'C:\Users\rmcgr\ntg_repository\tern\outputs/rfrLCHMcpickle_l8_99perc_20190407_512_n18_int32v', serialized=True)
    d['model'] = rfrLCHM
